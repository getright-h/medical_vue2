...
export default {
  ...
  methods: {
    ...
    async confirmAdd() {
      ...
      console.log(options);
      let lastAddress = ''
      const thisProvince =options.find(item => item.value === selectedOptions[0]).label
      console.log(thisProvince);
      const thisCity =options.find(item => item.children.value === selectedOptions[1]).label
      console.log(thisCity);
      const thisCounty =options.find(item => item.children.children.value === selectedOptions[2]).label
      console.log(thisCounty);
      lastAddress = thisProvince + '/' + thisCity + '/' + thisCounty
      let data = {
        address: lastAddress,
        bus: busLine,
        campus: name,
