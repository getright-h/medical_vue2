...
export default {
  ...
  methods: {
    //确定添加的回调
    async confirmAdd() {
      const { name, selectedOptions, phone1, phone2, busLine, options } =
        this.form;
      ...
      console.log(thisProvince.label);
      console.log(thisProvince.children[0].children);
      const thisCity = thisProvince.children[0].children.find((item) => {
        if (item.value === selectedOptions[1]) {
          return item;
        }
      });
      console.log(thisCity.label);
      // const thisCounty =options.find(item => item.value === selectedOptions[2]).label
      // console.log(thisCounty);
      let lastAddress = thisProvince + "/" + thisCity + "/" + thisCounty;
      let data = {
        address: lastAddress,
        bus: busLine,
        campus: name,
