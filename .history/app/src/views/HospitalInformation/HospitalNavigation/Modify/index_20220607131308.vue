<template>
  <el-dialog
    title="修改院区信息"
    :visible.sync="dialogUpFormVisible"
  >
    <el-form :model="form">
...
export default {
  ...
  methods: {
    async confirmUpdate() {
      const { name, upSelectedOptions, phone1, phone2, busLine, upOptions } = this.form;
      ...
      const thisCounty =thisCity.children.find(item => item.value === upSelectedOptions[2]).label
  ...
  watch: {
    thisData(){
      console.log('计算属性获取showdata',this.thisData);
      this.oneData = this.thisData
    }
  },
  mounted() {
    console.log(this.oneData);
    const { name = '', phone1 = '', phone2 = '', busLine = '',address = '' } = this.thisData;
    ...
    const thisProvinceNum = upOptions.find((item) => {
      ...
      const thisCounty =thisUpCity.children.find(item => item.label === address.split("/")[2]).label
    this.form.name = name
