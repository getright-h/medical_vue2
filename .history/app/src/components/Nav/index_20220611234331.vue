<template>
  <el-col :span="12">
    ...
    >
      ...
      <el-submenu :index="item.index" v-for="(item,index) in navData" :key="index">
        <template slot="title">
          <i class="el-icon-baseball"></i>
          <span>{{item.name}}</span>
        </template>
        <el-menu-item-group>
          <el-menu-item :index="little.router" v-for="(little,index) in item.children" :key="index">{{little.name}}</el-menu-item>
        </el-menu-item-group>
      </el-submenu>
      <!-- <el-submenu index="3">
        <template slot="title">
