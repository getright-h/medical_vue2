...
<template>
  <div class="pagination">
    <button @click="$emit('getPageNo', pageNo - 1,finishDate)" :disabled="pageNo == 1">
      上一页
    </button>
    <button
    ...
    <span v-for="(page, index) in startNumAndEndNum.end" :key="index"
      ><button
        ...
        @click="$emit('getPageNo', page,finishDate)"
      >
        {{ page }}
      </button></span
    >
    ...
    <button
      ...
      @click="$emit('getPageNo', totalPage,finishDate)"
    >
      {{ totalPage }}
    </button>
    <button
...
export default {
  ...
  computed: {
    ...
    finishDate(){
      ...
      console.log(pageNo);
      console.log((pageNo-1)*pageSize,((pageNo-1)*pageSize+pageSize));
      if(pageNo == totalPage){
      console.log((pageNo-1)*pageSize,(startDate.length-1));
      }
      return this.startDate.slice()
    }
  },
  mounted() {
    setTimeout(() => {
