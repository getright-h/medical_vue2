<template>
  <el-dialog title="修改套餐信息" :visible.sync="IsModify" :before-close="Cancel">
  ...
  export default {
	...
	watch:{
		...
		clickUpdateRow(v1,v2){
			console.log('v1',v1);
			this.ruleForm.comboId = v1.comboId//套餐ID
			this.ruleForm.id=v1.id//id
			this.ruleForm.name = v1.name //套餐名
			this.ruleForm.url = v1.url //套餐图片
			this.ruleForm.typeName = v1.typeName //套餐类型
			this.ruleForm.campus = v1.campus //所属院区
			this.ruleForm.items = v1.items //套餐项目
    ...
    methods: {
		...
		async confirm(){
			...
			let result= await this.$API.physicalExaminationModify(data);
			if(result.code === 200){
				this.$message({
					message: '恭喜你，修改成功',
					type: 'success'
