<template>
  <el-dialog title="新增医生信息" :visible.sync="IsAdd" :before-close="Cancel">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
			...
			<el-form-item label="挂号费" prop="registration">
				<el-input v-model="ruleForm.registration" type="number"></el-input>
			</el-form-item>
			<el-form-item label="诊察费" prop="examination">
				<el-input v-model="ruleForm.examination"></el-input>
			</el-form-item>
		...
		async confirm(){
			if(this.file){
				...
				if(resultUrl.code==200){
					...
					money:this.ruleForm.registration,	//挂号费
					money1:this.ruleForm.examination,	// 诊察费
					text: this.ruleForm.text,	//医生简介
					state: this.ruleForm.state,	//医生状态
					}
					// let result
					if(this.ruleForm.name&&this.ruleForm.prof&&this.ruleForm.department&&this.ruleForm.label&&this.ruleForm.registration&&this.ruleForm.examination&&this.ruleForm.text&&this.ruleForm.state){
						
						let result= await this.$API.doctorManagementAdd(data);
						if(result.code === 200){
							this.$message({
								message: '恭喜你，添加成功',
								type: 'success'
						...
						this.ruleForm.label=[]
						this.ruleForm.registration=60
						this.ruleForm.examination=0
						this.ruleForm.text=''
						this.ruleForm.state=''
						}else{
