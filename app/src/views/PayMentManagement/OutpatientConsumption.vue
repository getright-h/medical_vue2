    ...
    <template v-slot:mytable>
      <div>
        ...
        >
          ...
          <el-table-column prop="ctype" label="费用类型" width="120">
          </el-table-column>
...
export default {
  ...
  data() {
    return {
      navInfo: this.$store.state.zzl.tableData,
      tableData: [],
      thisData: {}, //传给修改子组件的该行信息
      // navInfo: this.tableData, //传给子组件用于分页处理的原始数据
      pageNo: 1, //当前页
      pageSize: 5, //每一页多少条数据
  ...
  mounted() {
    ...
    // 获取渲染的数据，从plugins中获取
    this.getTableData("/exrecord/queryOutpatient");
    //
    ...
    this.$bus.$emit("updateSelect", {
      ...
      type: "selectOutpatient",
      url: "/exrecord/selectOutpatient",
    });
  },
  methods: {
    //分页
    ...
    fn({ medcard, order_num }) {
      ...
      this.$router.push({
        ...
        params: {
          ...
          //操作接口的地址
          url: "/exrecord/oderDetails",
          remarkurl: "/exrecord/updateHosremark",
